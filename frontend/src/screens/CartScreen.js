        ...
        </li>
        {
          cartItems.length === 0 ?
            ...
            cartItems.map((item) => (
              ...
              {
                  item.unit==='Gram' || item.unit==='Gm' ?(<b> &#x20B9;{item.selling_price/4 * item.qty/250}</b>):null
                }
                {
                  item.unit==='KG' || item.unit==="Kg" || item.unit==="Pcs" || item.unit==="PCS" ?( <b> &#x20B9; {item.selling_price * item.qty}</b>):null
                }
